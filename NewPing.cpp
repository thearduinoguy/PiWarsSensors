...
unsigned long NewPing::ping_mm(unsigned int max_cm_distance) {
	float echoTime = NewPing::ping(max_cm_distance); // Calls the ping method and returns with the ping echo distance in uS.
    int distance_mm = (echoTime / US_ROUNDTRIP_CM)*100;
#if ROUNDING_ENABLED == false
	
	return distance_mm;              // Call the ping method and returns the distance in centimeters (no rounding).
#else
	return NewPingConvert(distance_mm); // Convert uS to centimeters.
#endif
}
...
unsigned long NewPing::ping_median(uint8_t it, unsigned int max_cm_distance) {
	...
	while (i < it) {
		...
		last = ping_mm(max_cm_distance);  // Send ping.
